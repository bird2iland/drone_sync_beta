...
## 目录示例
- `-p` 指向的路径下包含日期文件夹或直接包含机位文件夹：
- `项目/素材/251114/CAM1/CARD#A/clip1.MP4 + clip1.SRT`
- `项目/素材/251114/CAM2/CARD#B/clip3.MP4 + clip3.SRT`

## 使用
- 命令：
- `python3 drone_sync_beta_v1.py -p '<日期文件夹或其上级路径>' -f <帧率> -o '<导出XML目录>'`
- 示例：
- `python3 drone_sync_beta_v1.py -p '/Volumes/DIT-4T#10/素材' -f 50 -o '/Volumes/DIT-4T#10/export'`

## 交互说明
- 启动后先检测 `-p` 下的结构：
- 若存在机位文件夹（`CAM*`），直接列出供选择；
- 若不存在机位文件夹，则列出其下的日期子文件夹，先选择一个日期文件夹，再列出机位供选择；
- 机位多选输入示例：`1,3,4` 或 `1 3 4`；仅处理所选机位，避免创建无效 Bin/时间线。
